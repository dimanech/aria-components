export default class InputDependsFrom {
	/**
	 * InputDependsFrom
	 *
	...
	constructor(domNode) {
		this.input = domNode;
		this.form = InputDependsFrom.findParentForm(domNode);
		this.dependsFrom = [];
	}
		...
		this.dependsFrom.forEach(item => {
			if (typeof item === 'object') {
				...
				item.forEach(item => {
					...
					hasValidInGroup = InputDependsFrom.isDependencyResolved(item);
				});
				dependantsState.push(hasValidInGroup);
			} else {
				dependantsState.push(InputDependsFrom.isDependencyResolved(item));
			}
		});
	...
	static findParentForm(domNode) {
		...
		return parent.tagName === 'FORM' ? parent : InputDependsFrom.findParentForm(parent);
	}
